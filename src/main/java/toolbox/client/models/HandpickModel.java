...
public class HandpickModel implements IModel, IRetexturableModel, IModelCustomData {
	
	public static final ModelResourceLocation LOCATION = new ModelResourceLocation(
			new ResourceLocation(Toolbox.MODID, "handpick"), "inventory");

	public static final IModel MODEL = new HandpickModel();
	...
	public HandpickModel(ResourceLocation head, ResourceLocation haft, ResourceLocation handle, ResourceLocation adornment) {
	...
	public IModel retexture(ImmutableMap<String, String> textures) {
		...
		if (textures.containsKey("head")) {
			head = new ResourceLocation(Materials.head_registry.get(textures.get("head")).getModId(), "items/handpick/head_" + textures.get("head"));
		}
		if (textures.containsKey("haft")) {
			haft = new ResourceLocation(Materials.haft_registry.get(textures.get("haft")).getModId(), "items/handpick/haft_" + textures.get("haft"));
		}
		if (textures.containsKey("handle")) {
			handle = new ResourceLocation(Materials.handle_registry.get(textures.get("handle")).getModId(), "items/handpick/handle_" + textures.get("handle"));
		}
		if (textures.containsKey("adornment")) {
			adornment = new ResourceLocation(Materials.adornment_registry.get(textures.get("adornment")).getModId(), "items/handpick/adornment_" + textures.get("adornment"));
		}
		return new HandpickModel(head, haft, handle, adornment);
	}
	...
	public IModel process(ImmutableMap<String, String> customData) {
		...
		if (customData.containsKey("head")) {
			head = new ResourceLocation(Materials.head_registry.get(customData.get("head")).getModId(), "items/handpick/head_" + customData.get("head"));
		}
		if (customData.containsKey("haft")) {
			haft = new ResourceLocation(Materials.haft_registry.get(customData.get("haft")).getModId(), "items/handpick/haft_" + customData.get("haft"));
		}
		if (customData.containsKey("handle")) {
			handle = new ResourceLocation(Materials.handle_registry.get(customData.get("handle")).getModId(), "items/handpick/handle_" + customData.get("handle"));
		}
		if (customData.containsKey("adornment")) {
			adornment = new ResourceLocation(Materials.adornment_registry.get(customData.get("adornment")).getModId(), "items/handpick/adornment_" + customData.get("adornment"));
		}
		return new HandpickModel(head, haft, handle, adornment);
	}
	...
	public Collection<ResourceLocation> getTextures() {
		...
		for (HeadMaterial mat : Materials.head_registry.values()) {
			builder.add(new ResourceLocation(mat.getModId(), "items/handpick/head_" + mat.getName()));
		}
		for (HaftMaterial mat : Materials.haft_registry.values()) {
			builder.add(new ResourceLocation(mat.getModId(), "items/handpick/haft_" + mat.getName()));
		}
		for (HandleMaterial mat : Materials.handle_registry.values()) {
			builder.add(new ResourceLocation(mat.getModId(), "items/handpick/handle_" + mat.getName()));
		}
		for (AdornmentMaterial mat : Materials.adornment_registry.values()) {
			builder.add(new ResourceLocation(mat.getModId(), "items/handpick/adornment_" + mat.getName()));
		}
	...
	public enum LoaderHandpick implements ICustomModelLoader {
		INSTANCE;

		@Override
		public boolean accepts(ResourceLocation modelLocation) {
			return modelLocation.getResourceDomain().equals(Toolbox.MODID)
					&& modelLocation.getResourcePath().equals("handpick");
		}
	...
	private static final class BakedHandpickOverrideHandler extends ItemOverrideList {
		public static final BakedHandpickOverrideHandler INSTANCE = new BakedHandpickOverrideHandler();
	...
	private static final class BakedHandpickModel implements IPerspectiveAwareModel {

		private final HandpickModel parent;
		private final Map<String, IBakedModel> cache;
		private final ImmutableMap<TransformType, TRSRTransformation> transforms;
		private final ImmutableList<BakedQuad> quads;
		private final VertexFormat format;

		public BakedHandpickModel(HandpickModel parent, ImmutableList<BakedQuad> quads, VertexFormat format,
