...
public class HammerModel implements IModel, IRetexturableModel, IModelCustomData {
	
	public static final ModelResourceLocation LOCATION = new ModelResourceLocation(
			new ResourceLocation(Toolbox.MODID, "hammer"), "inventory");

	public static final IModel MODEL = new HammerModel();
	...
	public HammerModel(ResourceLocation head, ResourceLocation haft, ResourceLocation handle, ResourceLocation adornment) {
	...
	public IModel retexture(ImmutableMap<String, String> textures) {
		...
		if (textures.containsKey("head")) {
			head = new ResourceLocation(Materials.head_registry.get(textures.get("head")).getModId(), "items/hammer/head_" + textures.get("head"));
		}
		if (textures.containsKey("haft")) {
			haft = new ResourceLocation(Materials.haft_registry.get(textures.get("haft")).getModId(), "items/hammer/haft_" + textures.get("haft"));
		}
		if (textures.containsKey("handle")) {
			handle = new ResourceLocation(Materials.handle_registry.get(textures.get("handle")).getModId(), "items/hammer/handle_" + textures.get("handle"));
		}
		if (textures.containsKey("adornment")) {
			adornment = new ResourceLocation(Materials.adornment_registry.get(textures.get("adornment")).getModId(), "items/hammer/adornment_" + textures.get("adornment"));
		}
		return new HammerModel(head, haft, handle, adornment);
	}
	...
	public IModel process(ImmutableMap<String, String> customData) {
		...
		if (customData.containsKey("head")) {
			head = new ResourceLocation(Materials.head_registry.get(customData.get("head")).getModId(), "items/hammer/head_" + customData.get("head"));
		}
		if (customData.containsKey("haft")) {
			haft = new ResourceLocation(Materials.haft_registry.get(customData.get("haft")).getModId(), "items/hammer/haft_" + customData.get("haft"));
		}
		if (customData.containsKey("handle")) {
			handle = new ResourceLocation(Materials.handle_registry.get(customData.get("handle")).getModId(), "items/hammer/handle_" + customData.get("handle"));
		}
		if (customData.containsKey("adornment")) {
			adornment = new ResourceLocation(Materials.adornment_registry.get(customData.get("adornment")).getModId(), "items/hammer/adornment_" + customData.get("adornment"));
		}
		return new HammerModel(head, haft, handle, adornment);
	}
	...
	public Collection<ResourceLocation> getTextures() {
		...
		for (HeadMaterial mat : Materials.head_registry.values()) {
			builder.add(new ResourceLocation(mat.getModId(), "items/hammer/head_" + mat.getName()));
		}
		for (HaftMaterial mat : Materials.haft_registry.values()) {
			builder.add(new ResourceLocation(mat.getModId(), "items/hammer/haft_" + mat.getName()));
		}
		for (HandleMaterial mat : Materials.handle_registry.values()) {
			builder.add(new ResourceLocation(mat.getModId(), "items/hammer/handle_" + mat.getName()));
		}
		for (AdornmentMaterial mat : Materials.adornment_registry.values()) {
			builder.add(new ResourceLocation(mat.getModId(), "items/hammer/adornment_" + mat.getName()));
		}
	...
	public enum LoaderHammer implements ICustomModelLoader {
		INSTANCE;

		@Override
		public boolean accepts(ResourceLocation modelLocation) {
			return modelLocation.getResourceDomain().equals(Toolbox.MODID)
					&& modelLocation.getResourcePath().equals("hammer");
		}
	...
	private static final class BakedHammerOverrideHandler extends ItemOverrideList {
		public static final BakedHammerOverrideHandler INSTANCE = new BakedHammerOverrideHandler();
	...
	private static final class BakedHammerModel implements IPerspectiveAwareModel {

		private final HammerModel parent;
		private final Map<String, IBakedModel> cache;
		private final ImmutableMap<TransformType, TRSRTransformation> transforms;
		private final ImmutableList<BakedQuad> quads;
		private final VertexFormat format;

		public BakedHammerModel(HammerModel parent, ImmutableList<BakedQuad> quads, VertexFormat format,
