		...
		for (IRecipe recipe : CraftingManager.getInstance().getRecipeList()) {
			...
			if (recipe.getRecipeOutput().getItem() instanceof ItemHoe
				...
				if (recipe instanceof ShapelessRecipes) {
					List<ItemStack> inputs = ((ShapelessRecipes) recipe).recipeItems;
					List<ItemStack> newInputs = new ArrayList<ItemStack>();
					boolean flag = false;
					for (ItemStack stack : inputs) {
						...
						if (item instanceof ItemHoe || item instanceof ItemAxe || item instanceof ItemSpade
							...
							flag = true;
							newInputs.add(getToolReplacement(item));
						} else {
							newInputs.add(stack);
						}
					}
					if (flag) {
						toRemove.add(recipe);
						toAdd.add(new ShapelessNBTOreRecipe(recipe.getRecipeOutput(), newInputs.toArray()));
					}
				} else if (recipe instanceof ShapedRecipes) {
					ItemStack[] inputs = ((ShapedRecipes) recipe).recipeItems;
					ItemStack[] newInputs = new ItemStack[inputs.length];
